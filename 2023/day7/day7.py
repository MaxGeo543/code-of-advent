...
def rank_hand_jokers(hand):
    ...
    if  hand.count("J") == 5 or any(c != "J" and hand.count(c) + hand.count("J") == 5 for c in hand): # five of a kind
        ...
        hand_score = 6
    elif (hand.count("J") == 2 and any(c != "J" and hand.count(c) == 2 for c in hand)) or (hand.count("J") == 1 and [hand.count(c) for c in hand].count(2) == 4) or (any(hand.count(c) == 2 for c in hand) and any(hand.count(c) == 3 for c in hand)): # full house
        hand_score = 5
...
if __name__ == "__main__":
    ...
    assert part2(TEST_INPUT_PATH) == 5905

    part1(PUZZLE_INPUT_PATH) # returned 248396258
    part2(PUZZLE_INPUT_PATH) # returned 246436046